    ...
    Tensor LayerNorm::forward(Tensor& input) {
        ...
        return dispatchOne(input.dtype(), [&](auto TA) {
            ...
            case DeviceType::CPU:
                ...
                cpu_layer_normalization(iptr,optr,gptr,bptr,meanptr,invstdptr,num_batches,m_dim,m_eps);
                break;

            case DeviceType::CUDA:
                Logger::Append(Logger::LogLevel::LOGLEVEL_DEBUG, "CUDA Code path - Using normal layernorm path");
                cuda_layer_normalization(iptr,optr,gptr,bptr,meanptr,invstdptr,num_batches,m_dim,m_eps);
                break;
