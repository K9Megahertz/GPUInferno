...
namespace Inferno {
	...
	__global__ void softmax_strided_kernel(
		...
		for (size_t k = 0; k < axis_size; ++k) {
			...
			T e = static_cast<T>(exp((double)(aptr[aidx] - max_val)));
			optr[oidx] = e;
			sum += e;
		}
	...
	void cuda_softmax(
		...
		softmax_strided_kernel<T> << <blocks, threads >> > (
			aptr,
			optr,
			d_shape,
			d_astrides,
	...
	template void cuda_softmax<float>(
		const float*, float*,
	...
	template void cuda_softmax<double>(
		const double*, double*,
		const std::vector<size_t>&,
		const std::vector<size_t>&,
		const std::vector<size_t>&,
	...
	template void cuda_softmax<int>(
		const int*, int*,
		const std::vector<size_t>&,
		const std::vector<size_t>&,
		const std::vector<size_t>&,
	...
	template void cuda_softmax_backward<int, int, int>(
	...
	template void cuda_softmax_backward<int, float, float>(
	...
	template void cuda_softmax_backward<int, double, double>(
	...
	template void cuda_softmax_backward<float, int, float>(
	...
	template void cuda_softmax_backward<double, int, double>(
